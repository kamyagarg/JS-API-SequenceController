/* 
* @function: API call using async await.
...
async function asyncN(input_name) {

    //Fetching the link with given input
    let response = await fetch('https://api.nationalize.io/?name='.concat(input_name));

    //Converting the data into json format
    let data = await response.json();
    let req_info = data.country[0];
